...
async function handleGenerate() {
  if (!form.value.calculationBoundaryFile || !form.value.designDepthFile || !form.value.mudlineFile)
    return ElMessage.warning('请选择所有必需的文件！')

  loading.value = true
  const payload = { ...form.value, action: 'contour_point_cloud' }
  try {
    ...
    <el-row :gutter="40">
      ...
      <el-col :span="10">
        ...
        <div class="action-area">
          <el-button type="primary" @click="handleGenerate" style="width: 100%">开始生成</el-button>
        </div>
      </el-col>
    </el-row>
  </div>
